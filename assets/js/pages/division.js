...
const bodyTableDivision = document.getElementById("body-division")
const footerTable = document.querySelector('#foot-pagination');
const tableDivisionFolder = document.getElementById("table-division")
let totalColumn = countColumnTable(tableDivisionFolder)
getDataDivision(`/division`)
...
$('button[data-target="#divisionModal"]').click(function () {
    let data = $(this).data("division") === undefined ? undefined : $(this).data("division");
    // set title of modal
    $("#divisionModalLabel").text(
        typeof data !== "undefined" ? "Edit Position" : "Create Position"
    );

    // set data to form modal
    $("#divisionForm").attr("action", `/division`);
    $("#divisionForm").attr("method", `POST`);
    if (typeof data !== "undefined") {
        $('#divisionModal').modal('show');
        $("#divisionForm").attr("action", `/division/${data.id}`);
        $("#divisionForm").attr("method", `PUT`);
...
function getDataDivision(path) {
    bodyTableDivision.innerHTML = loadingTable(totalColumn);
    httpRequest(path, "GET", null, (res) => {
        if (res.response.code === 200) {
            ...
            if (data.data.length > 0) {
                data.data.forEach((v, i) => {
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Edit" data-division='${JSON.stringify(v)}' data-target="#divisionModal" data-toggle="modal"><i class="fa fa-edit"></i></button>
            ...
            bodyTableDivision.innerHTML = html;
